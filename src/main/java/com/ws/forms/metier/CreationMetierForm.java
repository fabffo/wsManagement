...
///     PROGRAMME FORM METIER  CREATION                                   ///
...
public final class CreationMetierForm {
	private MetierDao typeContratDao;
    ...
    public Metier creerMetier( HttpServletRequest request ) {
    	
    	DaoFactory daoFactory = DaoFactory.getInstance();
		this.typeContratDao = daoFactory.getMetierDao();
        
        String nom = getValeurChamp( request, CHAMP_NOM );
        String domaine = getValeurChamp( request, CHAMP_DOMAINE );
        
        Metier typeContrat = new Metier();
        ...
        typeContrat.setDomaine(domaine);     

        if ( erreurs.isEmpty() ) {
            resultat = "Succès de la création de la metier.";
        } else {
            resultat = "Échec de la création de la metier.";
        }
        return typeContrat;
    }
    ...
    private String validationNom( String nom ) throws Exception {
        String temp=nom;
        if ( temp != null ) {
                Boolean existe = typeContratDao.trouverNomMetier(nom);
                	if ( existe == true ) {
