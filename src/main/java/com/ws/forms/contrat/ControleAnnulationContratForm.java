...
public final class ControleAnnulationContratForm {
	...
	public ContratAncien AnnulerContrat(HttpServletRequest request, ContratAncien contrat) {
...
// =============================================================================================================//
// =====id contrat recherche de factures
		id = contrat.getId();
		try {
			valeurIdFacture = validationIdFacture(id);
		} catch (Exception e) {
	...
	private int validationIdFacture(int id) throws Exception {
		int temp = id;
		if (temp != 0) {
			if(contratDao.trouverFacture(temp)) {
				throw new Exception("le contrat ne peut être annulé car présence de facture(s) ");
			}else
				{}
		} else {
