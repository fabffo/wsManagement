	...
	<nav class="navbar navbar-expand-md bg-dark navbar-dark">
		...
		<div class="collapse navbar-collapse" id="collapsibleNavbar">
			<ul class="navbar-nav">
				...
				<li class="nav-item active"><a class="nav-link" href="gestionUtilisateur">Utilisateur</a></li>
			</ul>
		</div>
	</nav>
	<div class="container">
  ...
  <div class="row">
    <div class="col-md-6">
      ...
      <a class="btn btn-outline-dark  mb-3"
					href="ajoutUtilisateur?action=AJOUTER"><i class="bi bi-plus-square" data-toggle="tooltip" data-placement="top" title="AJOUTER"></i></a>
   
    </div>
    <div class="col-md-6">
      <!-- Barre de recherche à droite avec zone de sélection -->
      <form class="form-inline float-md-right" method="GET" action="gestionUtilisateur">
        <div class="form-group mr-2">
          <select class="form-control" id="critereUtilisateur" name="critereUtilisateur">
            <option ${sel_nom} value="nom">nom</option>
            <option  ${sel_libelle} value="libelle">Libelle</option>
          </select>
        </div>
        <div class="form-group mr-2">
          <input class="form-control" type="searchUtilisateur" name = "searchUtilisateur" placeholder="Recherche" aria-label="Search" value="${searchUtilisateur}">
        </div>
		...
		<table class="table table-striped">
			<thead class="thead-dark justify-content-center">
				<tr>
					<th class="col-md-1"><a href="gestionUtilisateur?triUtilisateur=ID&triUtilisateur_id=${triUtilisateur_id}"><i class="${triUtilisateur_id}"/></i></a>Id</th>
					<th class="col-md-1"><a href="gestionUtilisateur?triUtilisateur=NOM&tri_nomUtilisateur=${tri_nomUtilisateur}"><i class="${tri_nomUtilisateur}"/></i></a>Nom</th>
					<th class="col-md-3"><a href="gestionUtilisateur?triUtilisateur=LIBELLE&tri_libelleUtilisateur=${tri_libelleUtilisateur}"><i class="${tri_libelleUtilisateur}"/></i></a>Libelle</th>
					<th class="col-md-3">Actions</th>
				</tr>
			</thead>

			<tbody>
				<c:forEach var="utilisateur" items="${UtilisateurList}">
					<tr>
						<td><c:out value="${ utilisateur.id }" /></td>
						<td><c:out value="${ utilisateur.nom }" /></td>
						<td><c:out value="${ utilisateur.libelle }" /></td>
						<td><a
							href="majUtilisateur?id=${ utilisateur.id }&currentPage=${currentPage}"
							class="btn btn-outline-dark"><i class="bi bi-pencil-square" data-toggle="tooltip" data-placement="top" title="MAJ"></i></a>
							
							 <a
							href="copieUtilisateur?id=${ utilisateur.id }&currentPage=${currentPage}"
							class="btn btn-outline-dark"><i class="bi bi-book-half" data-toggle="tooltip" data-placement="top" title="COPIER"></i></a> 
							
							<a
							href="visualisationUtilisateur?id=${ utilisateur.id }"
							class="btn btn-outline-secondary" data-toggle="tooltip" data-placement="top" title="VISUALISER"><i class="bi bi-eye"></i></a> 
							
							<a
							href="suppressionUtilisateur?id=${ utilisateur.id }"
			...
			<c:set var="noBegin" value="${currentPage-pasPage+1}" />
				...
				<li class="page-item"><c:choose>
						<c:when test="${currentPage gt 1}">
							<a class="page-link"
								href="gestionUtilisateur?currentPage=${currentPage - 1}">Previous</a>
						</c:when>
						<c:otherwise>
						</c:otherwise>
					</c:choose>
				...
				<c:choose>
					<c:when test="${currentPage != i}">
						...
						href="gestionUtilisateur?currentPage=${i}">${i}</a></li>
					</c:when>
					<c:otherwise>
				...
				<c:if test="${currentPage lt noOfPages}">
					<li class="page-item"><a class="page-link"
						href="gestionUtilisateur?currentPage=${currentPage + 1}">Next</a></li>
				</c:if>
