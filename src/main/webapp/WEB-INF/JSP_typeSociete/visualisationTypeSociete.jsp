...
<head>
<meta charset="UTF-8">
<title>Visualisation type societe</title>
...
<body>
	...
	<div class="row flex-lg-nowrap w-80">
		<div
			...
			<h1 class="text-center btn btn-secondary">Visualisation type societe</h1>
			<div class="card-body">
				<form method="POST" action="gestionTypeSociete?action=VISUALISER">
					<div class="form-group row">
						<label for="id" class="col-auto col-form-label">Nom</label>
						<div class="col">
							<input type="text" class="form-control" name="id"
								value=<c:out value="${typeSociete.id}"/> disabled="disabled" />
						</div>
					</div>
					<div class="form-group row">
						...
						<div class="col">
							<input type="text" class="form-control" name="nom"
								value=<c:out value="${typeSociete.nom}"/> disabled="disabled" />
						</div>
					</div>
					<div class="form-group row">
						...
						<div class="col">
							<textarea type="text" class="form-control" name="libelle"
								disabled="disabled"><c:out
									value="${typeSociete.libelle}" /></textarea>
						</div>
					</div>
					
					<div class="box-footer">
						<a href=gestionTypeSociete?ACTION=ANNULER class="btn btn-danger">Annuler
						</a>
					</div>
				</form>
			</div>
		...
		<div
			...
			<div class="col">
			<p>	créé par <c:out value="${typeSociete.usercreation}"/> le 
				<c:out value="${typeSociete.datecreation}"/>
				par le pgm <c:out value="${typeSociete.pgmcreation}" />
					</p>
					</div>
								<div class="col">
			<p>	modifié par <c:out value="${typeSociete.usermodification}"/> le 
				<c:out value="${typeSociete.datemodification}"/>
				par le pgm <c:out value="${typeSociete.pgmmodification}" />
					</p>
					</div>
			</div>		</div>
